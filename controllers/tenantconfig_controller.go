...
func (r *TenantConfigReconciler) reconcileNormal(tenantConfigScope *scope.TenantConfigScope) (reconcile.Result, error) {
	tenantConfigScope.Info("Reconciling TenantConfig")
	...
	ownerName := GetOwnerAWSClusterName(tenantConfig.ObjectMeta)
	if ownerName == "" || tenantConfigScope.ClusterScope.AWSCluster.Name != ownerName {
		if err := controllerutil.SetControllerReference(tenantConfigScope.ClusterScope.AWSCluster, tenantConfig, r.Scheme); err != nil {
			return reconcile.Result{}, err
		}
		if err := tenantConfigScope.PatchObject(); err != nil {
			return reconcile.Result{}, err
	...
	if tenantConfigScope.ClusterScope.AWSCluster.Spec.TenantConfigName == tenantConfig.Name {
	...
	tenantConfig.Status.Ready = true
	return reconcile.Result{}, nil
}
