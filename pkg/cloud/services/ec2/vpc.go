...
func (s *Service) reconcileVPC() error {
	...
	if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
		...
		record.Warnf(s.scope.TenantConfig, "FailedTagVPC", "Failed to tag managed VPC %q: %v", vpc.ID, err)
...
func (s *Service) ensureManagedVPCAttributes(vpc *infrav1.VPCSpec) error {
	...
	if len(errs) > 0 {
		record.Warnf(s.scope.TenantConfig, "FailedSetVPCAttributes", "Failed to set managed VPC attributes for %q: %v", vpc.ID, err)
		return kerrors.NewAggregate(errs)
	}

	if updated {
		record.Eventf(s.scope.TenantConfig, "SuccessfulSetVPCAttributes", "Set managed VPC attributes for %q", vpc.ID)
	}
...
func (s *Service) createVPC() (*infrav1.VPCSpec, error) {
	...
	if err != nil {
		record.Warnf(s.scope.TenantConfig, "FailedCreateVPC", "Failed to create new managed VPC: %v", err)
		return nil, errors.Wrap(err, "failed to create vpc")
	}

	record.Eventf(s.scope.TenantConfig, "SuccessfulCreateVPC", "Created new managed VPC %q", *out.Vpc.VpcId)
	...
	if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
		...
		record.Warnf(s.scope.TenantConfig, "FailedTagVPC", "Failed to tag managed VPC %q: %v", *out.Vpc.VpcId, err)
		return nil, err
	}
	record.Eventf(s.scope.TenantConfig, "SuccessfulTagVPC", "Tagged managed VPC %q", *out.Vpc.VpcId)
...
func (s *Service) deleteVPC() error {
	...
	if _, err := s.scope.EC2.DeleteVpc(input); err != nil {
		...
		record.Warnf(s.scope.TenantConfig, "FailedDeleteVPC", "Failed to delete managed VPC %q: %v", vpc.ID, err)
		return errors.Wrapf(err, "failed to delete vpc %q", vpc.ID)
	}

	s.scope.V(2).Info("Deleted VPC", "vpc-id", vpc.ID)
	record.Eventf(s.scope.TenantConfig, "SuccessfulDeleteVPC", "Deleted managed VPC %q", vpc.ID)
	return nil
}
