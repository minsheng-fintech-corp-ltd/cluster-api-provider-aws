...
func (s *Service) reconcileSubnets() error {
	...
	defer func() {
		s.scope.TenantConfig.Spec.NetworkSpec.Subnets = subnets
	}()
		...
		for j := range subnets {
			...
			if (sn.ID != "" && exsn.ID == sn.ID) || (sn.CidrBlock == exsn.CidrBlock) {
				...
				if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
					...
					record.Warnf(s.scope.TenantConfig, "FailedTagSubnet", "Failed tagging managed Subnet %q: %v", exsn.ID, err)
...
func (s *Service) createSubnet(sn *infrav1.SubnetSpec) (*infrav1.SubnetSpec, error) {
	...
	if err != nil {
		record.Warnf(s.scope.TenantConfig, "FailedCreateSubnet", "Failed creating new managed Subnet %v", err)
		return nil, errors.Wrap(err, "failed to create subnet")
	}

	record.Eventf(s.scope.TenantConfig, "SuccessfulCreateSubnet", "Created new managed Subnet %q", *out.Subnet.SubnetId)
	...
	if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
		...
		record.Warnf(s.scope.TenantConfig, "FailedTagSubnet", "Failed tagging managed Subnet %q: %v", *out.Subnet.SubnetId, err)
		return nil, errors.Wrapf(err, "failed to tag subnet %q", *out.Subnet.SubnetId)
	}

	record.Eventf(s.scope.TenantConfig, "SuccessfulTagSubnet", "Tagged managed Subnet %q", *out.Subnet.SubnetId)
	...
	if sn.IsPublic {
		...
		if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
			...
			record.Warnf(s.scope.TenantConfig, "FailedModifySubnetAttributes", "Failed modifying managed Subnet %q attributes: %v", *out.Subnet.SubnetId, err)
			return nil, errors.Wrapf(err, "failed to set subnet %q attributes", *out.Subnet.SubnetId)
		}
		record.Eventf(s.scope.TenantConfig, "SuccessfulModifySubnetAttributes", "Modified managed Subnet %q attributes", *out.Subnet.SubnetId)
	}
...
func (s *Service) deleteSubnet(id string) error {
	...
	if err != nil {
		record.Warnf(s.scope.TenantConfig, "FailedDeleteSubnet", "Failed to delete managed Subnet %q: %v", id, err)
	...
	record.Eventf(s.scope.TenantConfig, "SuccessfulDeleteSubnet", "Deleted managed Subnet %q", id)
	return nil
}
