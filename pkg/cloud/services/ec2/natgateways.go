	...
	for _, sn := range s.scope.Subnets().FilterPublic() {
		...
		if ngw, ok := existing[sn.ID]; ok {
			...
			if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
				...
				record.Warnf(s.scope.TenantConfig, "FailedTagNATGateway", "Failed to tag managed NAT Gateway %q: %v", *ngw.NatGatewayId, err)
...
func (s *Service) createNatGateway(subnetID string) (*ec2.NatGateway, error) {
	...
	if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
		...
		record.Warnf(s.scope.TenantConfig, "FailedCreateNATGateway", "Failed to create new NAT Gateway: %v", err)
		return nil, errors.Wrapf(err, "failed to create NAT gateway for subnet ID %q", subnetID)
	}
	record.Eventf(s.scope.TenantConfig, "SuccessfulCreateNATGateway", "Created new NAT Gateway %q", *out.NatGateway.NatGatewayId)
	...
	if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
		...
		record.Warnf(s.scope.TenantConfig, "FailedTagNATGateway", "Failed to tag NAT Gateway %q: %v", *out.NatGateway.NatGatewayId, err)
		return nil, errors.Wrapf(err, "failed to tag nat gateway %q", *out.NatGateway.NatGatewayId)
	}

	record.Eventf(s.scope.TenantConfig, "SuccessfulTagNATGateway", "Tagged NAT Gateway %q", *out.NatGateway.NatGatewayId)
...
func (s *Service) deleteNatGateway(id string) error {
	...
	if err != nil {
		record.Warnf(s.scope.TenantConfig, "FailedDeleteNATGateway", "Failed to delete NAT Gateway %q previously attached to VPC %q: %v", id, s.scope.VPC().ID, err)
		return errors.Wrapf(err, "failed to delete nat gateway %q", id)
	}
	record.Eventf(s.scope.TenantConfig, "SuccessfulDeleteNATGateway", "Deleted NAT Gateway %q previously attached to VPC %q", id, s.scope.VPC().ID)
