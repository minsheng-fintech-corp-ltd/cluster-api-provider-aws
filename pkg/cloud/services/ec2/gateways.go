...
func (s *Service) reconcileInternetGateways() error {
	...
	if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
		...
		record.Warnf(s.scope.TenantConfig, "FailedTagInternetGateway", "Failed to tag managed Internet Gateway %q: %v", gateway.InternetGatewayId, err)
...
func (s *Service) deleteInternetGateways() error {
	...
	for _, ig := range igs {
		...
		if _, err := s.scope.EC2.DetachInternetGateway(detachReq); err != nil {
			record.Warnf(s.scope.TenantConfig, "FailedDetachInternetGateway", "Failed to detach Internet Gateway %q from VPC %q: %v", *ig.InternetGatewayId, s.scope.VPC().ID, err)
			return errors.Wrapf(err, "failed to detach internet gateway %q", *ig.InternetGatewayId)
		}

		record.Eventf(s.scope.TenantConfig, "SuccessfulDetachInternetGateway", "Detached Internet Gateway %q from VPC %q", *ig.InternetGatewayId, s.scope.VPC().ID)
		...
		if _, err = s.scope.EC2.DeleteInternetGateway(deleteReq); err != nil {
			record.Warnf(s.scope.TenantConfig, "FailedDeleteInternetGateway", "Failed to delete Internet Gateway %q previously attached to VPC %q: %v", *ig.InternetGatewayId, s.scope.VPC().ID, err)
			return errors.Wrapf(err, "failed to delete internet gateway %q", *ig.InternetGatewayId)
		}

		record.Eventf(s.scope.TenantConfig, "SuccessfulDeleteInternetGateway", "Deleted Internet Gateway %q previously attached to VPC %q", *ig.InternetGatewayId, s.scope.VPC().ID)
...
func (s *Service) createInternetGateway() (*ec2.InternetGateway, error) {
	...
	if err != nil {
		record.Warnf(s.scope.TenantConfig, "FailedCreateInternetGateway", "Failed to create new managed Internet Gateway: %v", err)
		return nil, errors.Wrap(err, "failed to create internet gateway")
	}
	record.Eventf(s.scope.TenantConfig, "SuccessfulCreateInternetGateway", "Created new managed Internet Gateway %q", *ig.InternetGateway.InternetGatewayId)
	...
	if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
		...
		record.Warnf(s.scope.TenantConfig, "FailedTagInternetGateway", "Failed to tag managed Internet Gateway %q: %v", *ig.InternetGateway.InternetGatewayId, err)
	...
	if err := wait.WaitForWithRetryable(wait.NewBackoff(), func() (bool, error) {
		...
		record.Warnf(s.scope.TenantConfig, "FailedAttachInternetGateway", "Failed to attach managed Internet Gateway %q to vpc %q: %v", *ig.InternetGateway.InternetGatewayId, s.scope.VPC().ID, err)
	...
	record.Eventf(s.scope.TenantConfig, "SuccessfulAttachInternetGateway", "Internet Gateway %q attached to VPC %q", *ig.InternetGateway.InternetGatewayId, s.scope.VPC().ID)
