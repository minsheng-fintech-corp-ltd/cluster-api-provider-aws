...
func (s *Service) allocateAddress(role string) (string, error) {
	...
	if err != nil {
		record.Warnf(s.scope.TenantConfig, "FailedAllocateEIP", "Failed to allocate Elastic IP for %q: %v", role, err)
...
func (s *Service) disassociateAddress(ip *ec2.Address) error {
	...
	if err != nil {
		record.Warnf(s.scope.TenantConfig, "FailedDisassociateEIP", "Failed to disassociate Elastic IP %q: %v", *ip.AllocationId, err)
...
func (s *Service) releaseAddresses() error {
	...
	for i := range out.Addresses {
		...
		if err != nil {
			record.Warnf(s.scope.TenantConfig, "FailedReleaseEIP", "Failed to disassociate Elastic IP %q: %v", *ip.AllocationId, err)
