...
func NewTenantConfigScope(params TenantConfigParams) (*TenantConfigScope, error) {
	...
	session, err := sessionForRegion(params.TenantConfig.Name, params.TenantConfig.Spec.Region, params.TenantConfig.Spec.RoleArn)
	if err != nil {
		return nil, errors.Errorf("failed to create aws session: %v, role: %v", err, params.TenantConfig.Spec.RoleArn)
	}
...
func (s *TenantConfigScope) AdditionalTags() infrav1.Tags {
	...
	for key, value := range s.TenantConfig.Spec.AdditionalTags {
		tags[key] = value
	}
	return tags
}
